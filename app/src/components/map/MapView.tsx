...
function getInfoWindowContent(place: MapPlace, placeUrl: string): string {
  ...
  return `
    <div style="font-family:system-ui,sans-serif;width:100%;min-width:300px;max-width:380px;box-sizing:border-box;overflow:hidden;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.12);border:2px solid ${borderColor};background:#fff;padding:0;">
      <div style="position:relative;height:140px;background:linear-gradient(135deg,${style.color},#fff);background-image:url('${img}');background-size:cover;background-position:center;flex-shrink:0;">
  ...
  useEffect(() => {
    ...
    Promise.all(imageUrls.map((url) => loadImageAsDataUrl(url))).then((imageDataUrls) => {
      ...
      marker.addListener('click', () => {
        ...
        onPlaceSelect?.(place)
        // CSS hozzáadása az InfoWindow-hoz: elrejtjük a beépített X gombot és csökkentjük a padding-et
        setTimeout(() => {
          ...
          if (container && typeof container === 'object') {
            ...
            const closeBtn = el.querySelector('.info-window-close-btn')
            if (closeBtn) {
