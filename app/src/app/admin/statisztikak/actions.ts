...
export async function getStatisticsForAdmin(
  ...
  try {
    ...
    if (placeIds.length > 0) {
      const { data: placeData } = await supabase.from('places').select('id, name').in('id', placeIds)
    ...
    const byPlace = placeIds
      ...
      .sort(
        (a, b) =>
          b.place_clicks + b.place_views + b.direction_clicks - (a.place_clicks + a.place_views + a.direction_clicks),
      )
