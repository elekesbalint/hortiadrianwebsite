...
function BejelentkezesForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(false)
    if (err) {
      setError(err.message === 'Invalid login credentials' ? 'Hibás e-mail vagy jelszó.' : err.message)
      return
    }
  ...
  const handleOAuth = async (provider: 'google' | 'facebook') => {
    ...
    if (err) {
      setError(err.message || 'A bejelentkezés indítása sikertelen.')
      return
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-black/5 border border-gray-100 overflow-hidden">
          <div className="p-8 sm:p-10">
            <h1 className="text-2xl font-bold text-[#1A1A1A] mb-1">Bejelentkezés</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <Link
                  ...
                  className="inline-block mt-2 text-sm font-medium text-[#2D7A4F] hover:text-[#1B5E20] transition-colors"
                >
                  Elfelejtett jelszó?
                </Link>
              </div>
          ...
          <div className="px-8 sm:px-10 py-4 bg-[#F8FAF8] border-t border-gray-100 text-center">
            <p className="text-sm text-[#6B7280]">
              Még nincs fiókod?{' '}
              <Link href="/regisztracio" className="font-semibold text-[#2D7A4F] hover:text-[#1B5E20] transition-colors">
                Regisztrálj
              </Link>
            </p>
          </div>
        ...
        <p className="mt-6 text-center text-xs text-[#6B7280]">
          A bejelentkezésével elfogadod az{' '}
          <Link href="/adatvedelem" className="text-[#2D7A4F] hover:underline">Adatvédelmi tájékoztató</Link>
          {' '}és a{' '}
          <Link href="/felhasznalasi-feltetelek" className="text-[#2D7A4F] hover:underline">Felhasználási feltételek</Link>
          {' '}elfogadását.
        </p>
      </div>
    </div>
